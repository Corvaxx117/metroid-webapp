...
abstract class BaseModel
{
    ...
    public function findAllWithStats(string $orderBy = 'date_creation', string $direction = 'DESC', array $stats = []): array
    {
        try {
             ...
             *   ]
             * ]
             *  dd($joinClauses, $selectFields, $orderBy, $direction);
             */
            $sql = "
            ...
            error_log("Erreur SQL dans findAllWithStats: " . $e->getMessage());
            return [];
        } catch (\Exception $e) {
            error_log("Erreur dans findAllWithStats: " . $e->getMessage());
            return [];
        }
    }
}
